...
void CorrectNVT(double DT,double XI_TRANS,double XI_ROT)
{
 ...
 for(i=0;i<NumberOfParticles;i++)
 {
  ...
  //p' = f - xi*p
  PX1I = force[i].x - XI_TRANS*PX[i];
  PY1I = force[i].y - XI_TRANS*PY[i];
  PZ1I = force[i].z - XI_TRANS*PZ[i];
  corrx = PX1I - PX1[i];
  corry = PY1I - PY1[i];
  corrz = PZ1I - PZ1[i];
 ...
 // WZ1I = (torque[i].z+WX[i]*WY[i]*(IXX[i]-IYY[i]))/IZZ[i];
  WX1I = torque[i].x/IXX[i] - XI_ROT*WX[i];
  WY1I = torque[i].y/IYY[i] - XI_ROT*WY[i];
  WZ1I = torque[i].z/IZZ[i] - XI_ROT*WZ[i];
  corrx = WX1I-WX1[i];
  corry = WY1I-WY1[i];
  corrz = WZ1I-WZ1[i];
